    ...
    layui.use('flow', function () {
        ...
        flow.load({
            ...
            scrollElem: '#house-flow',
                ...
                $.get('map/houses?cityEnName=' + cityName + '&start=' + start + '&size=3' + target,
                    function (res) {
                    console.log(res);
                        if (res.code !== 200) {
                            lis.push('<li>数据加载错误</li>');
                        } else {
                            layui.each(res.data.result, function (index, house) {
                                var direction;
                                ...
                                var li = '<li class="list-item"><a href="/house-service/rent/house/show/' + house.id + '" target="_blank"'
