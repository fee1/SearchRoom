...
$(function () {
    ...
    function submitSuccess(data) {
        if (data.code === 200) {
            ...
            layer.msg('登录已过期', {icon: 5, time: 2000});
        } else {
            layer.msg(data.message, {icon: 5, time: 2000});
        }
    }
    ...
    layui.use('flow', function () {
        ...
        flow.load({
            ...
            scrollElem: '.wait-record .content',
                ...
                $.get('house/subscribe/list?status=1' + '&start=' + start + '&size=3',
                    function (res) {
                        layui.each(res.result, function (index, tuple) {
    ...
    layui.use('flow', function () {
        ...
        flow.load({
            ...
            scrollElem: '.subscribe .content',
                ...
                $.get('house/subscribe/list?status=2' + '&start=' + start + '&size=3',
                    function (res) {
                        layui.each(res.result, function (index, tuple) {
                            var house = tuple.first,
    ...
    layui.use('flow', function () {
        ...
        flow.load({
            ...
            scrollElem: '.finish-record .content',
                ...
                $.get('house/subscribe/list?status=3' + '&start=' + start + '&size=3',
                    function (res) {
                        layui.each(res.result, function (index, tuple) {
                            var house = tuple.first,
                                subscribe = tuple.second;
