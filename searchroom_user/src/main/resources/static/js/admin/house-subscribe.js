...
var table = $('#data-table').DataTable({
        ...
        columnDefs: [{ // 定义表格样式
            ...
            targets: 8,
            render: function (data, type, row, meta) {
                return '<a style="text-decoration: underline" onclick="userTip(' + row.second.userId + ','+row.second.houseId+')">查看用户信息</a>'
            }
        }, {
            targets: 9,
            render: function (data, type, row, meta) {
...
function userTip(userId, houseId) {
    $.get('user/subscribe/detail/' + userId+'/'+houseId, function (res) {
        layer.open({
            type: 1,
            title: false, //不显示标题栏
            closeBtn: false,
